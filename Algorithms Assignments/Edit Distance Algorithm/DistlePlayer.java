...
public class DistlePlayer {
    ...
    public void getFeedback (String guess, int editDistance, List<String> transforms) {
    	Iterator<String> wordsLeft = dictionary.iterator();
    	while (wordsLeft.hasNext()) {
    		String word = wordsLeft.next();
    		List<String> transformsToCompare = getTransformationList(guess, word);
    		if (!transforms.equals(transformsToCompare)) {
    			wordsLeft.remove();
    		}
    	}
    } 
