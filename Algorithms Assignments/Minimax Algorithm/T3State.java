...
public class T3State {
    ...
    public Map<T3Action,T3State> getTransitions () {
        ...
        for (int x = 0; x < getOpenTiles().size(); x++) {
        	for (int y = 0; y < getMoves().length; y++) {
        		int[] index = getOpenTiles().get(x);
        		T3Action action = new T3Action(index[0], index[1], getMoves()[y]);
