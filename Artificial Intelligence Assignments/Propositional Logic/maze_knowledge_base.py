...
class MazeKnowledgeBase:
    ...
    def ask(self, query: "MazeClause") -> bool:
        ...
        """
        negate_of_query: "MazeClause" = MazeClause(
            [
                (proposition, not truth_value)
                for proposition, truth_value in query.props.items()
            ]
        )
        kb_copy: set["MazeClause"] = deepcopy(self.clauses)
        kb_copy.add(negate_of_query)
        new_clause: set["MazeClause"] = set()
        while True:
            combos: Iterator[tuple["MazeClause", "MazeClause"]] = itertools.combinations(kb_copy, 2)
            for combo in combos:
                first_clause: "MazeClause" = combo[0]
                second_clause: "MazeClause" = combo[1]
                resolvant: set["MazeClause"] = MazeClause.resolve(first_clause, second_clause)
                if MazeClause([]) in resolvant:
                    return True
                new_clause |= resolvant
            if new_clause.issubset(kb_copy):
                return False
            kb_copy |= new_clause
