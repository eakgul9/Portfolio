...
class ToxicityFilter:
    ...
    def __init__(self, text_train: pd.DataFrame, labels_train: pd.DataFrame):
        ...
        features: np.ndarray = self.vectorizer.fit_transform(text_train)
        self.model: MultinomialNB = MultinomialNB().fit(features, labels_train)
    ...
    def classify(self, text_test: list[str]) -> list[int]:
        ...
        """
        transformed_text: csr_matrix = self.vectorizer.transform(text_test)
        labels: list[int] = self.model.predict(transformed_text)
        return labels
    ...
    def test_model(
        ...
        """
        prediction: list[int] = self.classify(text_test.values.tolist())
        return (
